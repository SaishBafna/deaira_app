...
const ActivationPackage = () => {
  ...
  const [walletBalance] = useState(185.5); // Sample balance
  ...
  const packageOptions = [
    ...
    {
      value: "professional",
      label: "Professional Package - 300 USDT",
      price: 300,
    },
  ];
  ...
  const handleSubmit = (e) => {
    ...
    setMessage(
      `Purchasing ${packageDetails[formData.package].name} for ${
  ...
  return (
    <div className="w-screen h-screen bg-gradient-to-b from-[#1a0033] via-[#0c0c5f] to-[#00334d] relative overflow-y-auto flex flex-col items-center p-6 gap-6">
      ...
      {message && (
        <div data-testid="message" className="text-white text-center">
          {message}
        </div>
      )}
        ...
        {selectedPackage && (
          <div className="bg-[#1a1a40]/50 border border-purple-500/30 rounded-lg p-4">
            <h3 className="text-xl font-bold text-purple-300 mb-2">
              ...
              {packageDetails[selectedPackage].price} USDT
            </h3>
            <ul className="space-y-2">
              {packageDetails[selectedPackage].features.map(
                (feature, index) => (
        ...
        <div className="pt-4">
          <button
            ...
            disabled={
              !formData.package ||
              !formData.agreeTerms ||
              walletBalance < (packageDetails[selectedPackage]?.price || 0)
            }
      ...
      <div className="w-full max-w-4xl mt-2 mb-20 p-4 bg-[#504949] rounded-lg border border-gray-200 text-sm text-[#F64AFF]">
        <p className="mb-2">
          <span className="font-semibold">Note:</span> Packages are activated
          immediately after successful payment.
        </p>
        <p>
          For any issues, please contact our support team within 24 hours of
          purchase.
        </p>
      </div>
    </div>
